...
def evaluateRooms_cosine(benchmark_path, result_dir, num_of_classes=11, suffix=None, im_resize=True, gt_resize=True,
                         train=False):
    """
...
def evaluateBounds_cosine(benchmark_path, result_dir, suffix=None, im_resize=True, gt_resize=True, train=False):
    ...
    im_paths = [os.path.join(result_dir, p.split('/')[-1].split('.')[0]).replace('_close', '_doors_windows') + '.png'
                for p in d_paths]
    data_dir = os.path.dirname(benchmark_path)
    sims = []
    names = []
...
def evaluateBounds_pairWiseDistance(benchmark_path, result_dir, suffix=None, im_resize=True, gt_resize=True,
                                    train=True):
    """
...
def evaluate_semantic(benchmark_path, result_dir, num_of_classes=11, need_merge_result=False, im_downsample=True,
                      gt_downsample=True):
    gt_paths = open(benchmark_path, 'r').read().splitlines()
    d_paths = [p.split('\t')[2] for p in gt_paths]  # 1 denote wall, 2 denote door, 3 denote room
    r_paths = [p.split('\t')[3] for p in gt_paths]  # 1 denote wall, 2 denote door, 3 denote room
